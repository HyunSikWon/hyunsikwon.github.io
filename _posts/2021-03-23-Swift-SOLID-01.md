...
다시말해, OCP는 클래스에 새로운 기능을 추가/구현하는 것은 최소한의 노력과 변경이 있어야 한다는 원칙이다. 예제 코드를 통해 살펴보자. 먼저 `Logger` 클래스는 `Cars` 배열을 순회하며 각각의 세부 사항을 출력한다. 
...
이러한 접근 방식은 만약 클라이언트가 다음과 같은 유혀의 메소드를 가진다면  LSP의 원칙을 위반한다.
