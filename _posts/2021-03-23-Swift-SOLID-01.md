...
`SOLID`는 객체지향프로그래밍의 다섯가지 설계원칙인 **S**ingle responsibility(단일 책임 원칙), **O**pen/Closed(개방/폐쇄 원칙), **L**iskov Substitution(리스코프 치환 원칙), **I**nterface Segregation(인터페이스 분리 원칙), **D**ependency Inversion(의존관계 역전 원칙)의 줄임말이다. 이 원칙을 적용하면 나쁜 설계에서 나타나는 몇가지 문제들을 해결할 수 있다.
...
`Handler` 클래스는 API에서 데이터를 가져오고(1), 이 데이터를 문자열 배열로 파싱한 뒤(2), 이를 데이터베이스에 저장한다(3). 이렇게 `Handler` 클래스는 3가지 책임을 가지고 있다. 
...
> 베이스 클래스의 기능과 역할을 하위 클래스가 그것을 알지 않도 잘 사용할 수 있어야한다.
