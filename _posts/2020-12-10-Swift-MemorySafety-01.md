...
먼저, **읽기 접근과 쓰기 접근의 차이**는 분명하다. 쓰기 접근은 메모리의 위치를 변경하지만, 읽기 접근은 그렇지 않다. **메모리의 위치**는 액세스중인 항목 (변수, 상수 또는 프로퍼티)를 나타낸다. 그리고 **메모리 접근의 기간**은 **즉시 접근(instantaneous)**과 **장기 접근(long-term)**으로 나뉜다.
